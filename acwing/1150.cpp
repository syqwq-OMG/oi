...
// ===========================================================
// Problem: 秘密的牛奶运输
// URL: https://www.acwing.com/problem/content/1150/
// ===========================================================
cint N = 505, M = 1e4 + 5;
struct node {
    ll u, v, w;
...
void dfs(ll u, ll p, ll mx, ll e[]) {
    e[u] = mx;
    for (auto [v, w] : g[u])
        if (v != p) dfs(v, u, max(w, mx), e);
}
...
void solve() {
    ...
    rep(i, m) {
        ...
        ll x = get(u), y = get(v);
        if (x == y) continue;
        fa[x] = y, mst += w, edge[i].flg = 1;
        add(u, v, w), add(v, u, w);
    }
    rep(i, n) dfs(i, -1, 0, mxe[i]);
    ll ans = inf<ll>;
    rep(i, m) {
        auto [u, v, w, f] = edge[i];
        if (f) continue;
        if (w > mxe[u][v]) chmin(ans, mst - mxe[u][v] + w);
    }
    print(ans);
}

...
void solve() {
    cin >> n;
    rep(i, n) i & 1 ? cin >> a[i] : cin >> b[i];
    rep(i, n) i & 1 ? cin >> b[i] : cin >> a[i];

    unordered_map<ll, PII> ida, idb;
    per(i, n) {
        if (ida.count(a[i]) == 0) ida[a[i]] = {i, 0};
        elif (ida[a[i]].se == 0 && ida[a[i]].fi - i > 1) ida[a[i]].se = i;

        if (idb.count(b[i]) == 0) idb[b[i]] = {i, 0};
        elif (idb[b[i]].se == 0 && idb[b[i]].fi - i > 1) idb[b[i]].se = i;
    }

    int ans = 0;
    rep(i, n) if (idb.count(a[i])) chmax(ans, min(i, idb[a[i]].fi));

    per(i, n, max(ans, 1)) {
        if (ida[a[i]].se) chmax(ans, ida[a[i]].se);
        if (idb[b[i]].se) chmax(ans, idb[b[i]].se);
    }
    print(ans);
}

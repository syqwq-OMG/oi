...
void solve() {
    s[0] = 0;
    cin >> n;
    rep(i, n) cin >> a[i], s[i] = s[i - 1] + a[i], b[i] = 0;
    rep(i, n) if (s[i] == s[n] - s[i]) {
        print(1);
        aprint(a, 1, n);
        return;
    }
    ...
    if (a[idx] > s[n] - s[idx]) return print(-1);

    int t = s[n - 1] - s[1];
    if (t % 2 != 0) return print(-1);

    t /= 2;
    print(2);
    idx = 2;
    while (s[idx] - s[1] < t) idx++;
    // idx,idx+1
    rep(i, 2, idx) {
        if (i == idx) b[i] = t - (s[i - 1] - s[1]);
        else b[i] = a[i];
    }
    int j = idx + 1;
    while (t > 0 && j <= n) {
        if (a[j] <= t) b[j] = a[j], t -= a[j];
        else b[j] = t, t = 0;
        j++;
    }
    aprint(b, 1, n);

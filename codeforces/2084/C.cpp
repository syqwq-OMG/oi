...
void solve() {
    cin >> n;
    rep(i, n) cin >> a[i], ida[a[i]] = i;
    rep(i, n) cin >> b[i], idb[b[i]] = i;
    rep(i, n) if (idb[a[i]] != ida[b[i]]) return print(-1);
    ...
    ans.clear();
    if (same && same != (n + 1) / 2) change(same, (n + 1) / 2);
    rep(i, n / 2) {
        if (ida[a[i]] + idb[a[i]] == n + 1) continue;
        change(idb[a[i]], n + 1 - ida[a[i]]);
    }

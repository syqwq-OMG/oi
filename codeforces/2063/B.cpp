...
int n, l, r, sza, szb, szc, a[N], b[N], c[N];
void solve() {
    cin >> n >> l >> r;
    rep(i, n) a[i] = inf<int>, c[i] = inf<int>;
    sza = l - 1, szb = r - l + 1, szc = n - r;
    rep(i, sza) cin >> a[i];
    rep(i, szb) cin >> b[i];
    rep(i, szc) cin >> c[i];
    sort(all(a, sza)), sort(all(b, szb), greater<int>()), sort(all(c, szc));
    ll ans = inf<ll>;
    ll sum = 0ll;
    rep(i, szb) sum += min(a[i], b[i]);
    chmin(ans, sum);
    sum = 0ll;
    rep(i, szb) sum += min(b[i], c[i]);
    chmin(ans, sum);
    print(ans);
}

...
void add(vc<pid> *g, int u, int v, double w) { g[u].push_back({v, w}); }

void dijkstra(vc<pid> *g, int s) {
    rep(i, n) dis[i] = inf<double>;
...
int bfs(int s, int e, int mx) {
    auto f = [](int x, double d) { return dis[x] + d; };
